...
void rc_init0(void)
{
	...
	//
	rc_write_byte(REG_RC500_FIFO_LEVEL,0x20); 		//设置最多剩余字节数
...
void rc_iso14443_typea_init(void)
{
	...
	//2018/1/15 15:56:21 修改0x73值为0x72 2019/9/11 9:50:20再次修改为73
...
void rc_wait_irq(void)
{
 ...
 struct timespec delay, rem;
	...
	for(i=0;i<30000;i++)
	{
		...
		//7		(6 		5 		4) 		(3) 		(2) 		(1) 		(0)
		//0		ModemState  000发射器和接收器空闲
		//								IRQ表示Interrupen寄存器使能的任何一个中断源有请求
		// 											Err表示ErrorFlag寄存器理有错误状态
		// 														HiAlert表示FIFO中数据满足（64-FIFOLength)《WaterLevel
		//																	LoAlert表示FIFO中数据满足FIFOLength《WaterLevel
		if(sta&0x08) break;
...
#ifdef	DEBUG_CLOCK
	printf("rc_wait_irq %d clock_t %d\n\n", i, start);
#endif
	rc_clr_time_out();
...
//由于M1卡认证失败率较高，且返回3错误，调整wait_iqr函数终止条件
void rc_wait_m1_auth_irq(void)
{
	uint16_t i;
	uint8_t sta;
 struct timespec delay, rem;
...
uint8_t rece_bits,status;	
	...
	if(status != 0){
		#ifdef _DEBUG_RC500_REQUEST_
		//os_lock();
		sprintf(info_buf,"\x0d\x0a Err:request 1,status=%02X return 1\n",(uint8_t)status);
		printf(info_buf);
		//os_unlock();
		#endif
		return 1;
...
uint8_t rc_anticoll(uint8_t select_code,uint8_t bcnt,uint8_t *snr)    
{
	...
	//设置中断
	rc_clr_irq();
	rc_write_byte(REG_RC500_INTERRUPT_EN,(uint8_t)0xa8);  //允许超时中断和接收中断
	rc_set_time_out(10);
	rc_write_byte(REG_RC500_COMMAND,RC500_CMD_TRANSCEIVE);
	...
	if((rc_read_byte(REG_RC500_INTERRUPT_RQ) & 0x08) == 0){
		...
		os_unlock();
		#endif
		return (uint8_t)-1;    //无接收数据
	}
	
	if(colBit = rc_read_byte(REG_RC500_ERROR_FLAG)&0x01){
	    collpos = rc_read_byte(REG_RC500_COLL_POS);
	    //printf("************* have coll ,position =%d colBit =%0x\n", collpos, colBit);
	}
	...
	if((rc_read_byte(REG_RC500_ERROR_FLAG) & 0x0e) != 0){
		//printf("REG_RC500_ERROR_FLAG &0xE colbit=%x\n", colBit);
		if( colBit )
			return 127;
		return (uint8_t)-2;      //有错误发生
	}
...
uint8_t status,rece_bits;
	...
	buf[0] = PICC_WRITE;
	buf[1] = block;
	memcpy(&buf[2], inbuf, 4);
	rc_set_time_out(WRITE2_TIME_OUT);
	status = rc_send_cmd(buf,6,NORMAL_IRQ_MASK);
	if((status!=0)&&(status != STATUS_ACK_SUPPOSED)){
		 #ifdef _DEBUG_RC_WRITE_
...
uint8_t rc_auth(uint8_t keyAB,uint8_t sector)
{
	...
	rc_clr_irq();
	rc_write_byte(REG_RC500_INTERRUPT_EN,(uint8_t)BIT_SETI|BIT_IDLEI|BIT_TIMERI); //允许IDLE中断,超时中断
