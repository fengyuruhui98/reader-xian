...
void rc_set_time_out(uint16_t cnt_150us)
{
	//
	//7 	6	(5	4)	(3	2	1	0)
	//0 	0	TAutoResatart,timer递减到0后，自动重新从TReloadVlalue开始递减
	//					TPreScaler 定义timer时钟频率f，范围从0到21
	rc_write_byte(REG_RC500_TIMER_CLOCK,0x0b);	  //2048/13.56M = 151us
	//7		6	5	4 	(3) 	(2) 	(1) 	(0)
	//0		0	0	0	TStop-RxEnd 置1数据接收完毕后Timer自动停止
	//							TStopRxBegin 置1第一个正确的bit接收完Timer自动停止
	//									TStartTxEnd 置1数据发生接收后Timer自动期待，若已在运行则重新启动，从TReloadValue开始递减
	//											TStartTxBegin置1 第一个bit发送后Timer自动启动，若已运行则重新启动，从TReloadValue喀什递减
	rc_write_byte(REG_RC500_TIMER_CONTROL,0x06);   //2017/9/18 19:42:12数据传输完毕后自动运行  
	//TReloadValue，若Timer启动则从该值开始倒计数
...
int fp;  
    ...
    if ((fp = open(rcio, O_RDWR)) <= 0)   
    {  
        printf("Cannot open %s.\n",rcio);  
        return -1;  
    }  
    write(fp, act, strlen(act)); 
#ifdef RC_PRINT
    printf("reset pin : %s.\n",act);        
#endif
    close(fp);  
