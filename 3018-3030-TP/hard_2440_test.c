...
unsigned char outbytes;
	...
	*out_len = 1;
	...
	for(i = 0; i < outbytes; i++)  PRINTK("%02x ", (unsigned char)outbuf[i]); 
#endif
	if (outbuf[0] == 0x3b) 
	{
#ifdef DEBUG_PRINT
...
#ifdef DEBUG_PRINT
	PRINTK("\nantenna test-->");
	if(channel)  PRINTK("channel 1.");
	else PRINTK("channel 0.");
#endif
	*out_len = 1;
	rf_select(channel);
	mcml_pwr_on();
	...
	for(i = 0; i < try_times; i++)
	{
		...
		{//cpu
			...
			ret = mifpro_apdu(inbuf, 7, outbuf, &cpulen);
			if(ret != 0)
			{
#ifdef	DEBUG_PRINT
				PRINTK("     Err:select 3f 00\n");
#endif
				continue;
			}
#ifdef DEBUG_PRINT
