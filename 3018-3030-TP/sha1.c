...
//第1 页
//TAC 计算
//1. 将所有参与TAC 计算的交易数据做哈希运算，返回20 个字节计算结果，后补零后得24个字节数据。
//2. SAM 卡通用DES 计算初始化
//		80 1A 24 01 08 ＋ 交易序列号（4 个字节，按高字节在前，低字节在后规则) + 00 00 00 00
//3. SAM 卡通用DES 计算
//		80 FA 05 00 20 ＋ 00 00 00 00 00 00 00 00 ＋ 第一步计算出的哈希运算结果(24 个字节)
//返回4 个字节MAC 值作为TAC 值。/
//哈希算法
/* sha1.h
*
...
d' = c;
e' = d;
but this is implemented by unrolling the loop 5 times and renaming the variables ( e, a, b, c, d ) = ( a', b', c', d', e' ) each iteration.
...
static void sha1_transform(UINT32 *state, UINT32 *data)
{
...
static void sha1_block(struct sha1_ctx *ctx, const UINT8 *block)
{
...
void sha1_update(struct sha1_ctx *ctx, unsigned length, const UINT8 *buffer)
{
	if (ctx->index)
...
unsigned leftover;
	...
	if (leftover)
	{
		...
		// abort();
		case 3:
			digest[--j] = (UINT8)((word >> 8) & 0xff);
		/* Fall through */
		case 2:
			digest[--j] = (UINT8)((word >> 16) & 0xff);
		/* Fall through */
		case 1:
			digest[--j] = (UINT8)((word >> 24) & 0xff);
		}
	}
}